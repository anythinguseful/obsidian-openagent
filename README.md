...
| Structured memory engine | `src/agent/memoryEngine.ts` — a Hindsight-style typed-fact layer (plugin-native: no Docker/MCP/server). One LLM call distills each turn into world/experience facts with add·update·delete + dedupe; per-message recall fuses BM25 + entity + temporal + trust and — with an optional embedding model (e.g. `embedding-gemma-300m`, off by default, configured in Settings → Model with its own provider pin so recall can run on a local server while chat runs elsewhere) — cosine similarity over that provider's `/v1/embeddings`; observations join the recall block. A background `reflect` pass consolidates facts into evidence-backed observations and answers standing mental-model questions (settled knowledge read straight from disk, no LLM at retrieval). All engine files live in `<memory folder>/.engine/` |
...
**Model** (two peer pickers — the main chat model and the optional embedding model, each with its own provider/model route — plus custom model id, reasoning effort, temperature, max tokens, streaming) ·
