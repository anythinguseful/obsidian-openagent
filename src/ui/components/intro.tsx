...
const WORDMARK = "OPEN AGENT";
/**
 * Empty-state intro — full-fidelity port of Hermes Desktop
 * (apps/desktop/src/components/chat/intro.tsx + intro-copy.jsonl @main):
 * a big OPEN AGENT wordmark plus ONE rotating copy line. The copy pool is
 * the OFFICIAL jsonl verbatim (75 records, 15 personalities × 5); a key
 * with no pool falls back to the official generated "<Label> mode …"
 * templates. Copy selection: (random mount seed + intro seed) % length —
 * a new line per mount AND per fresh draft. Only `body` renders.
 */

export type IntroCopy = { headline: string; body: string };
export type IntroCopyRecord = IntroCopy & { personality: string };
...
/** Official NEUTRAL_PERSONALITIES */
const NEUTRAL_PERSONALITIES = new Set(["", "default", "none", "neutral"]);

/** Official FALLBACK_COPY, verbatim. */
...
/* Official parseIntroCopy: tolerant line-by-line JSON — a bad generated
   line must never break the app. */
function parseIntroCopy(raw: string): Record<string, IntroCopy[]> {
	...
	for (const line of raw.split(/\r?\n/)) {
		...
		try {
			...
			/* bad generated copy never breaks the app */
		}
	}
	return byPersonality;
}

/** Official intro-copy.jsonl, embedded verbatim (15 personalities × 5). */
const INTRO_COPY_JSONL = `{"personality":"helpful","headline":"Ready when you are","body":"Ask me to open a repo, run tests, fix a bug, or draft a PR. I'll walk through the steps with you."}
...
const INTRO_COPY_BY_PERSONALITY = parseIntroCopy(INTRO_COPY_JSONL);

function neutralCopy(): IntroCopy[] {
	return INTRO_COPY_BY_PERSONALITY.none || INTRO_COPY_BY_PERSONALITY.default || FALLBACK_COPY;
}

/** Official fallbackCopyForPersonality templates, verbatim. */
...
/* Testability export (official keeps pools private): every body the
   component can render, for harness assertions. */
export function introBodyPool(): string[] {
	const seen = new Set<string>();
	for (const pool of Object.values(INTRO_COPY_BY_PERSONALITY)) {
		for (const c of pool) seen.add(c.body);
	}
	for (const c of FALLBACK_COPY) seen.add(c.body);
	return [...seen];
}

export function Intro({ personality, seed: introSeed }: IntroProps) {
	/* official: random per mount + the caller's draft seed; each fresh
	   mount (chat started & left empty state) may rotate the line */
	const [mountSeed] = useState(() => Math.floor(Math.random() * 100000));
	const copy = resolveCopy(personality, mountSeed + (introSeed ?? 0));
	return (
		<div className="oa-intro">
			<p aria-label={WORDMARK} className="oa-intro-wordmark">
				{WORDMARK}
			</p>
			<p className="oa-intro-copy">{copy.body}</p>
		</div>
	);
}
