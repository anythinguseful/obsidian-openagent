/**
 * Quick Ask — CM6 glue (Copilot `src/editor/quickAskExtension.ts`,
 * ported with adaptations).
 *
 * A StateEffect carries show/hide commands; a ViewPlugin owns the
 * overlay instance, maps anchors through document changes, feeds the
 * ReplaceGuard, and rerenders the panel so the Replace button's
 * disabled state stays honest.
 *
 * NOTE: the selection highlight is managed by the controller, not here —
 * dispatching from inside ViewPlugin.update() is illegal ("Calls to
 * EditorView.update are not allowed while an update is in progress").
 * (Copilot hit and documented the same trap.)
 */

import { StateEffect } from "@codemirror/state";
import { EditorView, ViewPlugin, ViewUpdate } from "@codemirror/view";
import { mapQuickAskAnchorPositions } from "./anchors";
import { QuickAskOverlay, QuickAskOverlayOptions } from "./overlay";
		...
		update(update: ViewUpdate) {
			for (const tr of update.transactions) {
				for (const effect of tr.effects) {
					...
					this.topAnchorPos = typeof payload.topAnchorPos === "number" ? payload.topAnchorPos : null;
					this.focusAnchorPos = typeof payload.focusAnchorPos === "number" ? payload.focusAnchorPos : null;
			...
			if (this.overlay && update.docChanged) {
				/* guard first — it is the single source of truth for Replace */
				const guard = this.overlay.getReplaceGuard();
				if (guard?.onDocChanged) guard.onDocChanged(update.changes);
