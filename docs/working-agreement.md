...
### 206. (2026-08-24) Satu bug punya banyak bentuk — guard yang hanya melarang bentuk yang pertama ditemukan meninggalkan sisanya hidup

- Perbaikan awal `saveSettings` hanya menangani bentuk yang pertama terlihat: `await x.saveSettings()` di **ekor** callback UI. Audit AST lanjutan menemukan **bentuk lain dari bug yang sama persis**: 16 situs `void x.saveSettings()` (`void` membuang promise berikut rejection-nya) dan 1 situs `x.saveSettings().then(...)` tanpa `.catch` (rejection tetap lolos, dan blok `.then` — yang berisi refresh UI — justru dilewati tepat ketika penyimpanan gagal). Totalnya 17 kehilangan diam-diam yang lolos dari guard versi pertama. **Setelah memperbaiki satu bentuk, cari bentuk lain dari kesalahan yang sama sebelum menutup temuan.**
- Guard bentuk-1 dibatasi pada posisi **ekor** callback. Setelah 31 situs dikonversi, batasan itu menjadi celah: `await` yang bersarang di dalam `if`/`case` tidak lagi terjaga, padahal callback tetap membuang promise-nya. **Syarat posisi (\"pernyataan terakhir\") relevan untuk menilai keamanan konversi, tetapi TIDAK boleh menjadi syarat pada guard** — guard harus melarang bentuknya di mana pun ia berada.
- Celah itu ditemukan justru karena sebuah lengan red-proof **tetap hijau**. Sesuai Pelajaran 202, mutasinya diperiksa lebih dulu: ternyata mutasi mengenai situs `commit:` (property assignment, bukan `onChange`), jadi guard memang benar untuk kasus itu. Setelah mutasi diarahkan ke `onChange` sejati barulah celah posisi-ekor terbukti. **Lengan red-proof yang hijau adalah temuan, bukan gangguan — telusuri sampai tahu apakah yang salah mutasinya atau guard-nya.**
- Ditambahkan lengan red-proof **positif**: `.then(...).catch(...)` yang sah harus tetap **hijau**. Guard yang hanya diuji dengan kasus buruk bisa saja terlalu ketat tanpa ketahuan. **Buktikan dua arah: bentuk buruk merah, bentuk sah hijau.**
- Konversi 31 situs \"ada kode setelah await\" aman karena satu sifat yang diverifikasi lebih dulu, bukan diasumsikan: `display()` dan `refreshViews()` membaca dari **memori**, sedangkan mutasi state sudah terjadi **sebelum** penyimpanan. Hasil render identik baik penyimpanan berhasil maupun gagal. **Sebelum mengubah urutan await, buktikan apa yang dibaca oleh kode sesudahnya.**
- Lima situs `bare-call` sengaja **tidak** diubah: `() => this.saveSettings()` adalah *pass-through* yang meneruskan promise ke pemanggil (`ProfileStore`, hub, prop React), bukan pemanggilan yang membuangnya. **Bentuk yang mirip belum tentu bug — periksa siapa yang menerima promise-nya.**

### 207. (2026-08-24) `as` bukan validasi: data disk yang di-cast adalah janji yang tidak pernah ditepati

- `JSON.parse(raw) as Session` **tidak memeriksa apa pun**. Ia hanya memberi tahu `tsc` untuk berhenti bertanya. File sesi adalah data disk — tulisan terpotong, suntingan tangan, atau skema versi lama menghasilkan JSON valid dengan field wajib yang hilang. Akibatnya `tsc` hijau, tetapi runtime menghasilkan tiga crash dari satu akar: `session.turns is not iterable`, `turn.parts.map` atas `undefined`, dan `title.toLowerCase()` atas `undefined`. **Cast adalah asumsi; validasi adalah kode. Di setiap batas I/O (disk, jaringan, clipboard), yang dibutuhkan adalah kode.**
- Dampaknya melebar diam-diam karena akarnya ada di **titik baca bersama**. Satu file rusak tidak hanya membuat sesi itu gagal dibuka — `search()` mengiterasi *semua* sesi, jadi satu file menjatuhkan pencarian lintas sesi untuk seluruh riwayat, sekaligus jalur muat chat lewat `setTurnsSynced(s.turns)`. **Perbaiki di titik baca, bukan di tiap pemakai:** menambahkan `?? []` di pemanggil akan menambal satu gejala dan meninggalkan sisanya.
- Sanitasi harus **menormalkan, bukan menolak**, agar tidak menghancurkan data yang masih bisa dipakai: id non-string ditolak (`null`), `turns`/`parts` dipaksa array, `title`/`model`/timestamp diberi default, `turnCount` jatuh ke `turns.length`, dan seluruh field opsional (`messages`, `goal`, `todos`, `compression`, `personality`, `parent`) diteruskan apa adanya. Round-trip sesi normal dibuktikan identik lebih dulu — **perbaikan ketahanan yang diam-diam mengubah data sehat adalah bug baru.**
- Bug ini **direproduksi runtime** (bundel esbuild + adapter vault tiruan) sebelum disentuh, dan diverifikasi lagi sesudahnya. Membaca kode hanya memberi hipotesis; menjalankannya memberi nama error yang tepat — dan nama itulah yang membuat komentar serta pesan guard bernilai bagi orang berikutnya. **Bedakan fakta dari hipotesis dengan mengeksekusinya.**
- Lengan red-proof pertama gagal karena alasan yang salah: guard-nya benar, tetapi pola `JSON.parse(...) as Session` cocok dengan **kalimat di komentar penjelas yang baru saya tulis sendiri**. Sesuai Pelajaran 202, mutasi ditelusuri lebih dulu, bukan guard-nya yang dilonggarkan; komentarnya yang disusun ulang. **Guard yang memindai teks sumber ikut memindai prosa — dokumentasi yang mengutip bentuk terlarang akan memicu guard-nya sendiri.**

### 208. (2026-08-24) Bug yang senyap lebih berbahaya daripada bug yang crash — dan keduanya bisa lahir dari satu `JSON.parse`

- Kolom **Custom headers** menyimpan hasil `JSON.parse(v)` apa adanya. Mengetik `null` membuat render Settings berikutnya crash (`Object.keys(null)`) — kelihatan, langsung ketahuan. Mengetik `"halo"` **tidak** crash: string itu tersebar menjadi header per-karakter `{"0":"h","1":"a",…}` yang **ikut terkirim ke provider di setiap request**, tanpa satu pun gejala di UI. **Ketika satu input buruk punya cabang crash dan cabang senyap, cabang senyap yang harus dikejar lebih dulu** — yang crash akan dilaporkan pengguna, yang senyap tidak akan pernah.
- `JSON.parse` bukan validator bentuk: `null`, `123`, `"halo"`, `[1,2]` semuanya JSON valid. `typeof x === "object"` juga tidak cukup — `null` dan array lolos. Predikatnya harus utuh: bukan null, `typeof === "object"`, bukan array, **dan** setiap nilainya bertipe benar. (Bandingkan Pelajaran 207: di sana masalahnya `as`, di sini `JSON.parse` yang sukses. Akarnya sama — bentuk data tak pernah diperiksa.)
...
## Lesson 213 — (2026-08-25) Preflight release: cek versi di detik pertama, bukan setelah pipeline 3 menit; dan shim harness wajib contract-complete

**Konteks.** Release workflow `32758294248` gagal di step "Build complete verified asset set" setelah 3 menit 36 detik. Penyebab sebenarnya bukan versi basi (itu baru akan menggagalkan "Publisher dry run" di step 7), melainkan: (1) settings-preview harness crash di probe F17 karena shim plugin (`settings-entry.tsx`) tidak punya `saveSettingsSafe` (`moaSave` memanggilnya, `TypeError` → `display()` tak jalan → DOM re-render gagal → baris Reference 3 tak muncul → `page.evaluate` throw). (2) Probe F14 juga salah karena `buttons.includes("Apply")` memindai seluruh halaman, bukan baris Global default model — scope diperbaiki dengan memisahkan `assignmentButtons` (baris) dan `routeButtons` (route card). Kedua bug baru ketahuan karena harness browser **belum pernah dijalankan dengan Chromium asli** — sandbox sebelumnya tak bisa install Playwright.

**Pelajaran.**
1. **Preflight versi.** `release.mjs` sekarang cek di detik pertama: `git ls-remote --tags origin refs/tags/v{version}` — jika tag sudah ada, gagal cepat dengan "version X has already been released; bump before running release" tanpa menunggu typecheck/build/test/preview (3+ menit).
