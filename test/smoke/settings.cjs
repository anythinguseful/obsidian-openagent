...
module.exports = function settingsGuards() {
	...
	// v0.1.154: Settings grouping shells were reverted (owner: restore the
	// page from before grouping). Native subsection spacing remains; MCP
	// object cards stay. Absence pins so the boxed shells cannot return.
	{
		const tab = read("src/settingsTab.ts");
		const css = read("styles.css");
		const ok =
			...
			css.includes(".oa-settings .oa-mcp-server {") &&
